...
class TunerViewController: UIViewController {
    ...
    var tuner = SciTuner.sharedInstance
    ...
    let microphoneSwitch: UISwitch = {
        let microphoneSwitch = UISwitch()
        microphoneSwitch.addTarget(self, action: #selector(microphoneSwitchChanged(_:)), for: .valueChanged)
    ...
    override func viewDidLoad() {
        ...
        let microphoneSwitchContainer = UIStackView(arrangedSubviews: [UILabel(), microphoneSwitch])
        microphoneSwitchContainer.alignment = .center
        microphoneSwitchContainer.spacing = 8
        
        stackView.addArrangedSubview(microphoneSwitchContainer)
    ...
    @objc func microphoneSwitchChanged(_ sender: UISwitch) {
        if sender.isOn {
            microphone?.activate()
        } else {
            microphone?.inactivate()
