    ...
    var fret: Fret = .openStrings {
        didSet{
            if fret == .openStrings {
                ...
                fretMode.setTitleColor(.white, for: UIControl.State())
                fretMode.setTitle("tune on fret", for: UIControl.State())
                return
            }
            
            fretMode.backgroundColor = Style.highlighted0
            fretMode.setTitleColor(.white, for: UIControl.State())
            fretMode.setTitle(fret.localized(), for: UIControl.State())
        }
    }
    ...
    var filter: Filter = .on {
        didSet{
            if filter == .on {
                ...
                filterMode.setTitleColor(.white, for: UIControl.State())
            } else {
                filterMode.backgroundColor = Style.highlighted1
                filterMode.setTitleColor(.white, for: UIControl.State())
            }
            
            filterMode.setTitle("filter".localized() + ": " + filter.localized(), for: UIControl.State())
        }
    }
